...
suppressMessages({
  ...
  # create table version of crop diffs in future
  tab_kbs_crop_yield_change_df <- kbs_summary_output_piv[kbs_summary_output_piv$source %in% 
                                            c("Maize_Diff_Mgha","Soybean_Diff_Mgha",
                                              "Wheat_Diff_Mgha") & 
                                              kbs_summary_output_piv$Model %in%
                                              c("APSIM","Daycent"),] %>%
    select(Model,Climate_Scenario,scenario_abbrev,site_name,source,vals) %>%
    arrange(Climate_Scenario,Model,scenario_abbrev) %>%
    pivot_wider(names_from=scenario_abbrev,
                values_from=vals)
  
  tab_lrf_crop_yield_change_df <- lrf_summary_output_piv[lrf_summary_output_piv$source %in% 
                                                           c("Cotton_Diff_Mgha","Sorghum_Diff_Mgha") & 
                                                           lrf_summary_output_piv$Model %in%
                                                           c("APSIM","Daycent"),] %>%
    select(Model,Climate_Scenario,scenario_abbrev,site_name,source,vals) %>%
    arrange(Climate_Scenario,Model,scenario_abbrev) %>%
    pivot_wider(names_from=scenario_abbrev,
  ...
  ### write data frames
  write.csv(tab_kbs_crop_yield_change_df, 
            file=paste0(these_results_folder,"tab_kbs_crop_yield_change_df.csv"),
            row.names=FALSE)
  
  write.csv(tab_lrf_crop_yield_change_df, 
            file=paste0(these_results_folder,"tab_lrf_crop_yield_change_df.csv"),
            row.names=FALSE)
