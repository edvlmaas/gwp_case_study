...
This Readme file provides guidance for navigating the code base for the “Management alternatives for climate-smart agriculture in the U.S.: A model ensemble case study” paper by Ellen Maas and Debjani Sihi. It is divided into the following sections:
...
* Millennial – version 2. This model is shared with an MIT software license and has been included in this repository with permission from the author. There is no need to separately fetch and download the model, but for reference, this is the official link to the model: (https://github.com/rabramoff/Millennial/releases/tag/v2).
...
•	Create management input files for Millennial and RothC (using Daycent output) and run each
...
•	Graph and analyze Millennial and RothC output
